...
implements ProxyData
{
    ...
    public Socket openConnection(String hostname, int port, int connectTimeout, int readTimeout) throws IOException {
        ...
        int statusCode = Integer.parseInt(httpReponseFirstLine.substring(9, 12));
        if (statusCode == 200) {
            return sock;
        } else if (statusCode == 101) {
            SkStatus.logInfo("<strong>Proxy upgraded tunnel successfully</strong>");
            return sock;
        }
        ...
        throw new HTTPProxyException(httpReponseFirstLine.substring(13), statusCode);
    }
