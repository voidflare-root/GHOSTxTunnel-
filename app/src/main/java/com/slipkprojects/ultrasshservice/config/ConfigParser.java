...
public class ConfigParser
{
	...
	public static boolean convertInputAndSave(InputStream input, Context mContext)
		...
		try {
			SecurityGuard.enforceConfigAccess(mContext);
	...
	*/
	...
	static {
		mCrypto = Cryptor.initWithSecurityConfig(
			new SecurityConfig.Builder(ConfigSecrets.internalConfigCipherPassword()).build());
	}
	...
	private static InputStream decodeInput(InputStream in) throws Throwable {
		...
		try {
			...
			ByteArrayOutputStream out = new ByteArrayOutputStream();
			Cripto.decrypt(ConfigSecrets.configPassword(), new ByteArrayInputStream(byteArrayOut.toByteArray()), out);
