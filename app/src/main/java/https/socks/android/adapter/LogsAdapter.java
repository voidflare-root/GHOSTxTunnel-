		...
		View.OnTouchListener
{
		...
		final int position)
	{
		...
		try
		{
			...
			viewHolder.itemView.setBackgroundColor(ContextCompat.getColor(mContext, R.color.appBackground));
			viewHolder.textLog.setLinkTextColor(ContextCompat.getColor(mContext, R.color.colorPrimary));
			viewHolder.textLog.setText(stripLogColors(Html.fromHtml(text)));
			viewHolder.textLog.setTextColor(ContextCompat.getColor(mContext, R.color.appTextPrimary));
		}
		catch (Exception e)
		{
	...
	private Spanned stripLogColors(Spanned text) {
		SpannableString clean = new SpannableString(text);
		for (ForegroundColorSpan span : clean.getSpans(0, clean.length(), ForegroundColorSpan.class)) {
			clean.removeSpan(span);
		}
		for (BackgroundColorSpan span : clean.getSpans(0, clean.length(), BackgroundColorSpan.class)) {
			clean.removeSpan(span);
		}
		return clean;
	}
