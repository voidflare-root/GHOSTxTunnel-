...
public class TunnelManagerThread
    ...
    protected void conectar(String servidor, int porta) throws Exception {
        ...
        try {
            ...
            if (useProxy && cause.contains("Key exchange was not finished")) {
                SkStatus.logError("Proxy: Connection Lost - check server payload/host");
            } else {
    ...
    protected void startTunnelVpnService() throws IOException {
        ...
        try {
            if (isudpmode()) {
                serverAddr = servidorIP = getBestAddress(InetAddress.getAllByName(prefs.getString(Settings.UDP_SERVER, ""))).getHostAddress();
            } else {
        ...
        if (isServiceVpnRunning()) {
            ...
            if (tunnelManager != null) {
                SkStatus.logInfo("VPN service already running, restarting tunnel route");
                tunnelManager.restartTunnel(m_socksServerAddress);
            } else {
                SkStatus.logInfo("VPN service is starting, waiting for tunnel route");
            }
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                mContext.startForegroundService(startTunnelVpn);
            } else if (mContext.startService(startTunnelVpn) == null) {
                SkStatus.logInfo("failed to start tunnel vpn service");
                throw new IOException("Vpn Service failed to start");
            }
        } catch (Exception e) {
            SkStatus.logException("VPN service start failed", e);
            throw new IOException("Vpn Service failed to start", e);
        }
    ...
    public static InetAddress getBestAddress(InetAddress[] inetAddressArr) throws UnknownHostException {
