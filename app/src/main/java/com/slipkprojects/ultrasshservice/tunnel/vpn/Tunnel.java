...
public class Tunnel
{
		...
		try
		{
			...
			for (String dns : dnsResolver) {
				try {
					...
					addRouteAddress(dns, forwardDns);
			...
			while (it.hasNext()) {
				...
				routeMessage.append(String.format("%s/%d", ip.isIPv4() ? ip.getIPv4Address() : ip.getIPv6Address(), ip.networkMask));
				routeMessage.append(", ");
			}
	...
	private void addRouteAddress(String address, boolean include) {
		try {
			...
			if (inetAddress instanceof Inet6Address) {
				mRoutes.addIPv6((Inet6Address) inetAddress, 128, include);
			} else {
				mRoutes.addIP(new CIDRIP(inetAddress.getHostAddress(), 32), include);
			}
	...
	private boolean routeThroughTunnel(final String socksServerAddress, final String[] dnsResolver, boolean forwardDns, final String udpResolver, final boolean transparentDns)
	{
		...
		if (forwardDns) {
			...
			mPdnsd.setOnPdnsdListener(new Pdnsd.OnPdnsdListener(){
					...
					@Override
					public void onStop(){
						// DNS helper can restart/exit on some ROMs; keep the VPN route alive.
					}
				});
